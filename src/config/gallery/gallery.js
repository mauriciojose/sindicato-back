    ...
    async init(){
        ...
        const storage = multer.diskStorage({
            destination: (req, file, cb) => {
                ...
                const dir = `./src/uploads/gallery/${path}`
                fs.access(dir, error => {
                if (error) {
    ...
    async resizeImages (req, res, next) {
        ...
        await Promise.all(
          req.files.map(async file => {
            ...
            .toFile(`./src/uploads/gallery/${req.body.path}/${output}200x300.webp`);
          ...
          .toFile(`./src/uploads/gallery/${req.body.path}/${output}100x100.webp`);
      
            req.body.files.push(file.filename);
            req.body.filesExt.push(file.filename+path.extname(file.originalname));
          })
        );
...
module.exports = GalleryConfiguration;